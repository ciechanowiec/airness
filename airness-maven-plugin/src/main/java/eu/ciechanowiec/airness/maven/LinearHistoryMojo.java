...
import eu.ciechanowiec.airness.governance.LinearHistoryCheck;
...
/**
 * The history is linear, so no commit in it joins more than one parent.
 *
 ...
 */
@Mojo(name = "linear-history", defaultPhase = LifecyclePhase.PACKAGE, threadSafe = true)
public final class LinearHistoryMojo extends AbstractRepositoryMojo {
    ...
    private List<Findings> checked(Path root) {
        ...
        this.getLog().info("Linear history read " + check.scanned() + " commit(s)");
        return check.findings();
    }

    private List<Findings> unborn() {
        this.getLog().info("The repository has no commits yet, so there is no history to shape");
        return List.of();
    }
}
