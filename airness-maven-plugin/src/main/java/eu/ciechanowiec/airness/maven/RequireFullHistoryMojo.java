 ...
 *
 * <p>A truncated clone disarms the commit-history check, the commit-typography check, the linear-history
 * check, and any history-wide secret scan all at once, and it disarms them silently: the commits that were
 * never fetched pass by not existing, and four checks report clean over a few dozen commits. This runs at
